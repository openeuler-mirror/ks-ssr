...
#include "src/ui/ui_file-protection.h"
#include "ssr-i.h"
...
namespace FP
{
FileProtection::FileProtection(QWidget *parent) : Page(parent),
                                                  m_ui(new Ui::FileProtection())
{
    m_ui->setupUi(this);
    ...
    connect(m_ui->m_fileTable, &FileTable::filesUpdate, this, &FileProtection::updateTips);
}
...
QString FileProtection::getNavigationUID()
{
    return tr("File protected");
}

QString FileProtection::getSidebarUID()
{
    return "";
}

QString FileProtection::getSidebarIcon()
{
    return "";
}

int FileProtection::getSelinuxType()
{
    return 0;
}

void FileProtection::searchTextChanged(const QString &text)
{
    m_ui->m_fileTable->searchTextChanged(text);
}

void FileProtection::addProtectedFile(bool checked)
{
...
bool FileProtection::checkTrustedLoadFinied(bool initialized)
{
    // 可信未初始化完成，不允许操作
    if (!initialized)
    {
...
bool FileProtection::isExistSelectedItem()
{
...
void FileProtection::popDeleteNotify(bool checked)
{
    RETURN_IF_TRUE(!checkTrustedLoadFinied(m_fileProtectedProxy->initialized()))
    ...
    connect(unprotectNotify, &TableDeleteNotify::accepted, this, &FileProtection::removeProtectedFiles);
}

void FileProtection::removeProtectedFiles()
{
...
void FileProtection::updateTips(int total)
{
    // 更新表格右上角提示信息
