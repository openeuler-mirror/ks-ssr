                                                      ...
                                                      m_refreshTimer(nullptr)
{
    ...
    connect(m_refreshTimer, &QTimer::timeout, this, &ExecuteProtected::updateRefreshIcon);
    ...
    connect(m_ui->m_executeTable, &ExecuteProtectedTable::filesUpdate, this, &ExecuteProtected::updateTips);
}

ExecuteProtected::~ExecuteProtected()
{
    delete m_ui;
}
...
bool ExecuteProtected::isExistSelectedItem()
{
...
void ExecuteProtected::searchTextChanged(const QString &text)
{
    m_ui->m_executeTable->searchTextChanged(text);
}

void ExecuteProtected::addExecuteFile(bool checked)
{
...
void ExecuteProtected::updateExecuteList(bool checked)
{
    m_refreshTimer->start(100);
    m_ui->m_executeTable->updateInfo();
}

void ExecuteProtected::recertification(bool checked)
{
...
void ExecuteProtected::popDeleteNotify(bool checked)
{
    ...
    connect(deleteNotify, &TableDeleteNotify::accepted, this, &ExecuteProtected::removeExecuteFiles);
}

void ExecuteProtected::removeExecuteFiles()
{
    QStringList fileList;
    auto trustedInfos = m_ui->m_executeTable->getExecuteRecords();
    for (auto trustedInfo : trustedInfos)
...
void ExecuteProtected::updateRefreshIcon()
{
    static int count = 0;
    count++;
