                                                    ...
                                                    m_refreshTimer(nullptr)
{
    ...
    connect(m_refreshTimer, &QTimer::timeout, this, &KernelProtected::updateRefreshIcon);
    ...
    connect(m_ui->m_kernelTable, &KernelProtectedTable::filesUpdate, this, &KernelProtected::updateTips);
}
...
bool KernelProtected::isExistSelectedItem()
{
...
void KernelProtected::searchTextChanged(const QString &text)
{
    m_ui->m_kernelTable->searchTextChanged(text);
}

void KernelProtected::addKernelFile(bool checked)
{
...
void KernelProtected::updateKernelList(bool checked)
{
    m_refreshTimer->start(100);
    m_ui->m_kernelTable->updateInfo();
}

void KernelProtected::recertification(bool checked)
{
...
void KernelProtected::popDeleteNotify(bool checked)
{
    ...
    connect(deleteNotify, &TableDeleteNotify::accepted, this, &KernelProtected::removeKernelFiles);
}

void KernelProtected::removeKernelFiles()
{
    QStringList fileList;
    auto trustedInfos = m_ui->m_kernelTable->getKernelRecords();
    for (auto trustedInfo : trustedInfos)
...
void KernelProtected::prohibitUnloading(bool status, const QString &path)
{
...
void KernelProtected::updateRefreshIcon()
{
    static int count = 0;
    count++;
