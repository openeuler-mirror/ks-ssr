...
function do_prepare()
{
    ...
    if [ "x$param" == "xupgrade" ]; then
        ...
        if [ "x$param" == "xrollback" ]; then
            /sbin/chroot $SYSROOT /bin/bash -c "mount -a"
...
{
    local ret=$SUCCESS
    local key_files=("/var" "/run" "/usr" "/etc" "/boot")
...
function do_rollback()
{
	...
	if [ $line == "$SYSROOT$OSBAK/boot" -o $line == "$SYSROOT$OSBAK/usr" -o \
	    $line == "$SYSROOT$OSBAK/run" -o $line == "$SYSROOT$OSBAK/etc" -o \
...
function do_upgrade()
{
    ...
    /sbin/chroot $SYSROOT /usr/bin/bash -c "bash /opt/ks-ssr/ks-ssr/do_upgrade.sh"
    if [ "$?" -ne 0 ];then
...
function mount_bind()
{
    local mount_bind_dirs=("/dev" "/sys" "/run" "/proc")
...
function check_fstab_devs()
{
	...
	# grep " /opt " make sure we find excat mount_point in /proc/mounts
        mount | grep " `realpath $SYSROOT$mount_dir` " > /dev/null 2>&1
        if [ $? -eq 0 ]; then
            continue
        else
...
function remount_efi_for_rescue()
{
    ...
    if [ "x$mode" == "xrescue" ] && [ "$mount_point" == "/boot/efi" ]; then
        mount | grep " `realpath $SYSROOT$mount_point` " > /dev/null 2>&1
        if [ $? -eq 0 ]; then
...
function part_mount()
{
    ...
    mount | grep " `realpath $mount_point` " > /dev/null 2>&1
    if [ $? -eq 0 ]; then
    ...
    case $cmd in
        ...
        rollback)
            ...
            if [ "${retval}" -eq 0 ];then
                ...
                /sbin/chroot $SYSROOT /bin/bash -c "bash /opt/ks-ssr/ks-ssr/rebuild_grub.sh"
                sync
            fi
            return "${retval}"
            ;;
            ...
            if [ "${retval}" -eq 0 ]; then
                rm -rf "$KS_SSR_LOCK" > /dev/null 2>&1
                /sbin/chroot $root_path /bin/bash -c "bash /opt/ks-ssr/ks-ssr/rebuild_grub.sh"
                sync
            fi
            return "${retval}"
            ;;
