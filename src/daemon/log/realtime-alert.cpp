...
bool RealTimeAlert::initIPSetMonitor()
{
    ...
    {
        KLOG_ERROR() << "Failed to add iptables rule, nmap attack dectection is disable";
        return false;
    }
    // 流程参考 ipset 源码
    ipset_load_types();
...
上面的 audit log 中，有两个事件， 此处监听 "/var/run/audispd_events" 套接字， 每次可写时代表一个事件， 所以每次调用此函数时只有一个事件， 事件中可以有多个 record， type 开头， 每一行为一个 record，每条 record 中有多个 field， field 以空格分割，当 record 中有 key 为 "ks-ssr" 开头时就认为关注的事件发生， 故发送 DetectHazard 信号。
*/
...
void RealTimeAlert::processIPSetData()
{
    ...
    Log::Manager::m_logManager->DetectHazard(ATTACK_DETECT, nmapAttackers.join(','));
    char flushRes[] = "flush ks-ssr-ip-set";
    ipset_parse_line(m_ipset, flushRes);
}

void RealTimeAlert::processAuditData(QSocketDescriptor socket, QSocketNotifier::Type activationEvent)
{
    ...
    emit Log::Manager::m_logManager->DetectHazard(HAZARD_BEHAVIOR, alertMsgList.join(','));
}
};  // namespace Log
};  // namespace KS