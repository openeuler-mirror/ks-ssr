...
namespace KS
{
DeviceDBus::DeviceDBus(QObject *parent) : QObject(parent)
{
...
{
    // 构造函数中DeviceManager还未初始化，所以放到这里赋值
...
QString DeviceDBus::GetInterfaces()
{
    ...
    for (int type = INTERFACE_TYPE_USB; type < INTERFACE_TYPE_UNKNOWN; ++type)
    {
        QJsonObject jsonObj{
            {KSC_DI_JK_TYPE, type},
...
QString DeviceDBus::GetInterface(int type)
{
    ...
    auto deviceRuleManager = DeviceRuleManager::instance();
    for (int interfaceType = INTERFACE_TYPE_USB; interfaceType < INTERFACE_TYPE_UNKNOWN; interfaceType++)
    {
        if (type == interfaceType)
        {
            QJsonObject jsonObj{
                {KSC_DI_JK_TYPE, interfaceType},
                {KSC_DI_JK_ENABLE, deviceRuleManager->isIFCEnable(interfaceType)}};
