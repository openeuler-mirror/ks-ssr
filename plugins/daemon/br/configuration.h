...
public:
    ...
    // 删除加固项的自定义参数
    bool delCustomRA(const QString& name);
    void delAllCustomRA();
    // 获取定时扫描时间
    int getTimeScan();
    // 设置定时扫描时间
    ...
    // 加载历史加固参数文件
    QSharedPointer<Protocol::ReinforcementHistory> readRhFromFile(const QString& path);
    // 写历史加固参数文件
    bool writeRhToFile(QSharedPointer<Protocol::ReinforcementHistory> rh, const QString path);
    // 设置历史加固参数
