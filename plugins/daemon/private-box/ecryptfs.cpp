...
{
EcryptFS::EcryptFS(QObject *parent)
    : QObject(parent)
{
...
QString EcryptFS::encrypt(const QString &umountPath, bool isForce)
{
    QString cmd = isForce ? QString("umount -lf %1").arg(umountPath) : QString("umount %1").arg(umountPath);
    execute(cmd);
