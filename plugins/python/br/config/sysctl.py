...
class KeyRebootSwitch:
    ...
    def open(self):
        ...
        if not os.path.exists('/etc/systemd/system/ctrl-alt-del.target'):
            br.utils.subprocess_not_output("ln -s /usr/lib/systemd/system/reboot.target /etc/systemd/system/ctrl-alt-del.target")

    def close(self):
        if  os.path.exists('/etc/systemd/system/ctrl-alt-del.target'):
            br.utils.subprocess_not_output("rm -rf /etc/systemd/system/ctrl-alt-del.target")
