...
class ResourceLimits:
    def __init__(self):
        ...
        self.conf_stack = br.configuration.KV(HISTORY_SIZE_LIMIT_CONF_BASHRC_PATH)
    ...
    def get(self):
        ...
        stack_file_output = br.utils.subprocess_has_output('grep -r \"unlimited\" {0} | grep stack'.format(RESOURCE_LIMITS_CONF_PATH))
        rss_file_ouput = br.utils.subprocess_has_output('grep -r \"unlimited\" {0} | grep rss'.format(RESOURCE_LIMITS_CONF_PATH))
    ...
    def set(self, args_json):
        ...
        if args['enabled']:
            ...
            if not os.path.exists(PAM_CHECK_PATH) or self.get_selinux_status():
                br.utils.subprocess_not_output("sed -i '/{0}/d' {1}".format('ulimit', HISTORY_SIZE_LIMIT_CONF_BASHRC_PATH))
            ...
            if self.get_selinux_status() and len(br.utils.subprocess_has_output("semodule -l |grep br-ulimit")) == 0:
                br.utils.subprocess_not_output("semodule -i {0}".format(SELINUX_MODULES_ULIMIT_PATH))
...
class HistorySizeLimit:
    def __init__(self):
        self.conf = br.configuration.KV(HISTORY_SIZE_LIMIT_CONF_PATH, "=", "=")
        self.conf_profile = br.configuration.KV(HISTORY_SIZE_LIMIT_CONF_PROFILE_PATH, "=", "=")
        self.conf_bashrc = br.configuration.KV(HISTORY_SIZE_LIMIT_CONF_BASHRC_PATH, "=", "=")

    def get(self):
        retdata = dict()

        # 获取HISTSIZE的值
        get_br_config = self.conf.get_value(HISTORY_SIZE_LIMIT_CONF_KEY_HISTSIZE)
        ...
        br.log.debug('get_br_config',get_br_config,'get_profile = ',get_profile,'get_profile_export = ',get_profile_export,'get_bashrc = ',get_bashrc,'get_bashrc_export = ',get_bashrc_export)
        br.log.debug('int(get_profile)|int(get_profile_export)|int(get_bashrc_export)|int(get_bashrc) = ',int(get_profile)|int(get_profile_export)|int(get_bashrc_export)|int(get_bashrc))
        if int(get_br_config) == int(get_profile)|int(get_profile_export)|int(get_bashrc_export)|int(get_bashrc):
            retdata[HISTORY_SIZE_LIMIT_CONF_KEY_HISTSIZE] = int(get_br_config)
            return (True, json.dumps(retdata))
        else:
