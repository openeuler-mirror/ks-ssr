...
class Firewall:
    def __init__(self):
        ...
        br.utils.subprocess_not_output("iptables-restore < {0}".format(IPTABLES_RULES_SAVE_PATH))

    def save_iptables_rule(self):
        br.utils.subprocess_not_output("iptables-save > {0}".format(IPTABLES_RULES_SAVE_PATH))
    ...
    def set_iptables(self,iptables_set_cmd,iptables_check_cmd,set_name,set_type):
        ...
        br.log.debug(br.utils.subprocess_has_output_ignore_error_handling(check_cmd))
    ...
    def del_iptables_history(self,target,iptablestype = "INPUT"):
        count = 0
        output_result = str(br.utils.subprocess_has_output(FIND_IPTABLES_INPUT  + " {0} -n --line-numbers".format(iptablestype) + " |grep {0} ".format(target)))
        for line in output_result.splitlines():
            br.log.debug("output_result.splitlines() = ",line)
            count = count + 1
        for i in range(count):
            br.log.debug("index = ",str(br.utils.subprocess_has_output(FIND_IPTABLES_INPUT  + " {0} -n --line-numbers".format(iptablestype) + " |grep {0} ".format(target))).splitlines()[0].split(' ')[0])
            br.utils.subprocess_not_output('iptables -D {0} {1}'.format(iptablestype,str(br.utils.subprocess_has_output(FIND_IPTABLES_INPUT   + " {0} -n --line-numbers".format(iptablestype) + " |grep {0} ".format(target))).splitlines()[0].split(' ')[0]))
...
class FirewallManager(Firewall):
    def get(self):
        ...
        retdata['threat-port'] = len(br.utils.subprocess_has_output_ignore_error_handling(CHECK_IPTABLES_INPUT_TCP + " --dport 21 -j REJECT")) == 0 or len(br.utils.subprocess_has_output_ignore_error_handling(CHECK_IPTABLES_INPUT_UDP + " --dport 21 -j REJECT")) == 0
        # 设置tcp或udp都符合
        retdata['tcp-udp'] = "tcp"
        # 是否清空规则与最大连接数由用户决定，默认为否,都为符合
        retdata['clear-configuration'] = False
        retdata['input-ports-connect-nums'] = 0
        retdata['disable-ping'] = len(br.utils.subprocess_has_output_ignore_error_handling(CHECK_IPTABLES_INPUT_ICMP + " " + TIMESTAMP_ECHO_REQUEST)) == 0
...
class IcmpTimestamp(Firewall):
    def get(self):
        retdata = dict()
        iptable_input = br.utils.subprocess_has_output_ignore_error_handling(CHECK_IPTABLES_INPUT_ICMP + " " + TIMESTAMP_REQUEST_DETAIL)
        iptable_output = br.utils.subprocess_has_output_ignore_error_handling(CHECK_IPTABLES_INPUT_ICMP + " " + TIMESTAMP_REPLY_DETAIL)
...
class Traceroute(Firewall):
    def get(self):
        retdata = dict()
        iptable_input = br.utils.subprocess_has_output_ignore_error_handling(CHECK_IPTABLES_INPUT_ICMP + " " + TRACEROUTE_DETAIL)
        iptable_output = br.utils.subprocess_has_output_ignore_error_handling(CHECK_IPTABLES_OUTPUT_ICMP + " " + TRACEROUTE_DETAIL)
        # br.log.debug(command_output)
