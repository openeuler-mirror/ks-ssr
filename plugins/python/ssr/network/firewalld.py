...
class Firewall:
    ...
    def set_iptables(self, iptables_set_cmd, iptables_check_cmd, set_name, set_type):
        ...
        ssr.log.debug(
            ssr.utils.subprocess_has_output_ignore_error_handling(check_cmd))
    ...
    def del_iptables_history(self, target, iptablestype="INPUT"):
        count = 0
        output_result = str(ssr.utils.subprocess_has_output(
        ...
        for i in range(count):
            ssr.log.debug("index = ", str(ssr.utils.subprocess_has_output(
                FIND_IPTABLES_INPUT + " {0} -n --line-numbers".format(iptablestype) + " |grep {0} ".format(target))).splitlines()[0].split(' ')[0])
            ssr.utils.subprocess_not_output('iptables -D {0} {1}'.format(iptablestype, str(ssr.utils.subprocess_has_output(
                FIND_IPTABLES_INPUT + " {0} -n --line-numbers".format(iptablestype) + " |grep {0} ".format(target))).splitlines()[0].split(' ')[0]))
...
class FirewallManager(Firewall):
    def get(self):
        ...
        retdata['threat-port'] = len(ssr.utils.subprocess_has_output_ignore_error_handling(CHECK_IPTABLES_INPUT_TCP + " --dport 21 -j REJECT")
                                     ) == 0 or len(ssr.utils.subprocess_has_output_ignore_error_handling(CHECK_IPTABLES_INPUT_UDP + " --dport 21 -j REJECT")) == 0
        # 设置tcp或udp都符合
        retdata['tcp-udp'] = "tcp"
        # 是否清空规则与最大连接数由用户决定，默认为否,都为符合
        retdata['clear-configuration'] = False
        retdata['input-ports-connect-nums'] = 0
        retdata['disable-ping'] = len(ssr.utils.subprocess_has_output_ignore_error_handling(
...
class IcmpTimestamp(Firewall):
    def get(self):
        retdata = dict()
        iptable_input = ssr.utils.subprocess_has_output_ignore_error_handling(
            CHECK_IPTABLES_INPUT_ICMP + " " + TIMESTAMP_REQUEST_DETAIL)
        iptable_output = ssr.utils.subprocess_has_output_ignore_error_handling(
