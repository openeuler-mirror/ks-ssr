...
class Sysctl:
    def get_items_by_pattern(self, partten):
        output = ssr.utils.subprocess_has_output(
            '{0} -a -r {1}'.format(SYSCTL_PATH, partten))
        lines = output.split('\n')
        retval = list()
        for line in lines:
            fields = line.split('=')
...
class IcmpRedirect(Sysctl):
    def get(self):
        retdata = dict()
        redirect_items = self.get_items_by_pattern(
            SYSCTL_ACCEPT_REDIRECTS_PATTERN)
    ...
    def set(self, args_json):
        ...
        redirect_items = self.get_items_by_pattern(
            SYSCTL_ACCEPT_REDIRECTS_PATTERN)
        sysctl_config = ssr.configuration.KV(
            SYSCTL_CONFI_FILE, SYSCTL_CONFIG_FIELD_PARTTERN, ' = ')
        enabled = args['enabled']

        for redirect_item in redirect_items:
            sysctl_config.set_all_value(
                redirect_item[0], "1" if enabled else "0")
...
class SourceRoute(Sysctl):
    def get(self):
        retdata = dict()
        source_route_items = self.get_items_by_pattern(
            SYSCTL_ACCEPT_SOURCE_ROUTE_PATTERN)
...
class SynFlood(Sysctl):
    def get(self):
        retdata = dict()
        retdata['enabled'] = (ssr.utils.subprocess_has_output(
            'sysctl -a -r net.ipv4.tcp_syncookies -n') == '1')
        return (True, json.dumps(retdata))
    ...
    def set(self, args_json):
        ...
        enabled = args['enabled']
        sysctl_config.set_all_value(
            "net.ipv4.tcp_syncookies", "1" if enabled else "0")
