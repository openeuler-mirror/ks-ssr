...
class USB(DRIVERS):
    ...
    def get(self):
        retdata = dict()
        retdata[USB_ARG_ENABLED] = self.status() or (len(ssr.utils.subprocess_has_output(
    ...
    def set(self, args_json):
        ...
        if not self.flag_usb:
            output = ssr.utils.subprocess_has_output_ignore_error_handling(
        ...
        if args[TTYS_ARG_ENABLED]:
            while open_index < output_nums:
                ...
                command_open_status = '{0} | grep unknown'.format(
                    flag_open_cmd)
                flag_open = ssr.utils.subprocess_has_output(
                    command_open_status)
            ...
            while close_index < output_nums:
                ...
                command_close_status = '{0} | grep unknown'.format(
                    flag_close_cmd)
                flag_close = ssr.utils.subprocess_has_output(
