...
class SSHD:
    def __init__(self):
        ...
        self.conf_ciphers = ssr.configuration.PAM(
            SSHD_CONF_PATH, "Ciphers\\s+")
        self.conf_protocol = ssr.configuration.PAM(
...
class SshdService(SSHD):
    def clear_port(self):
        ...
        for port in port_list:
            if port == "22" or port == "":
                continue
            ssr.utils.subprocess_has_output_ignore_error_handling(
...
class SftpUser(SSHD):
    ...
    def set(self, args_json):
        ...
        if args["enabled"]:
            ...
            if self.get_selinux_status():
                ssr.utils.subprocess_not_output(
                    "semodule -r ssr-sshd-sftp &> /dev/null")
        # 重启服务生效
        self.service.reload()
        return (True, '')
