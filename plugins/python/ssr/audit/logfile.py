...
LOGFILE_ROTETE_CONF = '/var/log/messages\n{\n    missingok\n\
...
class Permissions:
    ...
    def get(self):
        ...
        output = ssr.utils.subprocess_has_output('grep -r {0} {1}'.format('/var/log/messages','/etc/logrotate.d/syslog'))
        if len(output) == 0:
    ...
    def set(self, args_json):
        ...
        if args[PERMISSIONS_ARG_APPEND_PERMISSIONS_LIMIT]:
            output = ssr.utils.subprocess_has_output('grep -r {0} {1}'.format('/var/log/messages','/etc/logrotate.d/syslog'))
            if len(output) == 0:
                ssr.utils.subprocess_not_output('echo \'{0}\'    >> {1}'.format( LOGFILE_ROTETE_CONF, '/etc/logrotate.d/syslog'))
        else:
            ssr.utils.subprocess_not_output('sed -i \'s/{0}/{1}/g\' {2}'.format(LOGFILE_ROTETE_CONF, ' ', '/etc/logrotate.d/syslog'))
        return (True, '')