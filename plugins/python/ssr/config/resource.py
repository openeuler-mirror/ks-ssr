...
class ResourceLimits:
    def __init__(self):
        ...
        self.conf_stack = ssr.configuration.KV(
    ...
    def get(self):
        ...
        stack_file_output = ssr.utils.subprocess_has_output(
            'grep -r \"unlimited\" {0} | grep stack'.format(RESOURCE_LIMITS_CONF_PATH))
        rss_file_ouput = ssr.utils.subprocess_has_output(
...
class HistorySizeLimit:
    def __init__(self):
        ...
        self.conf_profile = ssr.configuration.KV(
            HISTORY_SIZE_LIMIT_CONF_PROFILE_PATH, "=", "=")
        self.conf_bashrc = ssr.configuration.KV(
            HISTORY_SIZE_LIMIT_CONF_BASHRC_PATH, "=", "=")
    ...
    def get(self):
        ...
        ssr.log.debug('get_ssr_config', get_ssr_config, 'get_profile = ', get_profile, 'get_profile_export = ',
                      get_profile_export, 'get_bashrc = ', get_bashrc, 'get_bashrc_export = ', get_bashrc_export)
        ssr.log.debug('int(get_profile)|int(get_profile_export)|int(get_bashrc_export)|int(get_bashrc) = ', int(
